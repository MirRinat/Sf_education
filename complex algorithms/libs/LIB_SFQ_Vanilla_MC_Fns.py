...
def selectDriftAndVol(ticker):
    ...
    calibratedDrift = (medDrift + weeklyDrift[-1]) / 2
    medianVol = np.median(weeklyVols)
    calibratedVol = (medianVol + weeklyVols[-1]) / 2
    return calibratedVol,calibratedDrift
...
def calculateSelectedPercentile(equity, currentPrice):
    setPrice = equity.getLastSelectedPrice()
    direction = equity.getLastDirection()
    cPercentile = equity.getLastSelectedPercentile()
    lastPurchasePrice = equity.getLastPurchasedPrice()
    if (direction == DIRECTION_LONG):
        longReturn = currentPrice - lastPurchasePrice
        if (setPrice > currentPrice):
            return cPercentile, longReturn
        return cPercentile - 3, longReturn
    if (direction == DIRECTION_SHORT):
        shortReturn = lastPurchasePrice - currentPrice
        if (setPrice < currentPrice):
            return cPercentile, shortReturn
        return cPercentile + 3, shortReturn


def printLog(equity, aReturn):
    fileName = r"..\Logs\{}.csv".format(equity.ticker)
    with open(fileName, 'a+') as fileHandler:
        dataLine = '{},{},{},{},{}\n'
        dlFormat = dataLine.format(equity.getLastSelectedPrice(), equity.getLastPurchasedPrice(),
                                   equity.getLastDirection(), equity.getLastSelectedPercentile(), aReturn)
        fileHandler.write(dlFormat)